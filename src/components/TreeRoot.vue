...
export default {
  ...
  setup(props, { emit }) {
    const emitEventToParent = (name, args) => {
      console.log("emitEventToParent", name, args);
      emit(name, ...args);
    };

    onMounted(() => {
      emitter.on("node:selected", args =>
        emitEventToParent("node:selected", args)
      );
      emitter.on("node:dragging", args =>
        emitEventToParent("node:dragging", args)
      );
      emitter.on("node:dragging:start", args => {
        emitEventToParent("node:dragging:start", args);
      });
      emitter.on("node:dragging:finish", args => {
        console.log("args", args);
        emitEventToParent("node:dragging:finish", args);
      });
      emitter.on("tree:mounted", args =>
        emitEventToParent("tree:mounted", args)
      );
      emitter.on("tree:filtered", args =>
        emitEventToParent("tree:filtered", args)
      );
    });

    onUnmounted(() => {
      emitter.off("node:selected", args =>
        emitEventToParent("node:selected", args)
      );
      emitter.off("node:dragging", args =>
        emitEventToParent("node:dragging", args)
      );
      emitter.off("node:dragging:start", args =>
        emitEventToParent("node:dragging:start", args)
      );
      emitter.off("node:dragging:finish", args =>
        emitEventToParent("node:dragging:finish", args)
      );
      emitter.off("tree:mounted", args =>
        emitEventToParent("tree:mounted", args)
      );
      emitter.off("tree:filtered", args =>
        emitEventToParent("tree:filtered", args)
      );
    });
  }
